...
use crate::install::{InstallOutcome, ensure_assets_fresh, install};
use crate::manifest::Manifest;
use crate::pipeline;
use crate::verify::{Severity, verify};
...
/// The CLI entry point, living in the library so every other module can
/// stay crate-private. `src/main.rs` is a shim over this.
pub fn run() -> Result<()> {
    let cli = Cli::parse();
    match cli.command {
        None => preprocess(),
